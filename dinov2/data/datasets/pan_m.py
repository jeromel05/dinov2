...
class PanM(ExtendedVisionDataset):
    ...
    def _load_extra(self, extra_path: str) -> np.ndarray:
        ...
        for (
            ...
            lmdb_env_labels = lmdb.open(lmdb_path_labels, readonly=True, meminit=False)
            lmdb_env_imgs = lmdb.open(lmdb_path_imgs, readonly=True, meminit=False)
            lmdb_env_meta = lmdb.open(lmdb_path_meta, readonly=True, meminit=False)
            ...
            # save img txn from which to get imgs later
            self._lmdb_txns[lmdb_path_imgs] = lmdb_txn_imgs

            meta_cursor = lmdb_txn_meta.cursor()
            label_cursor = lmdb_txn_meta.cursor()
            local_idx = 0
            for (key_meta, value_meta), (key_label, value_label) in zip(meta_cursor, label_cursor):
                entry = dict()
                entry["index"] = key_meta.decode()
                value_meta = ast.literal_eval(value_meta.decode())
                curr_num_ch = len(value_meta["channel_names"])
                entry["num_ch"] = curr_num_ch
                entry["fov"] = value_meta["fov"]

                if self.with_targets:  # TODO: Load ground truth
                    entry["class_id"] = int(value_label.decode())
                entry["lmdb_imgs_file"] = lmdb_path_imgs

                accumulated.append(entry)
                local_idx += 1

            self._file_ch_list.append(curr_num_ch)
            self._file_len_list.append(local_idx)
    ...
    def _dump_entries(self) -> None:
        split = self.split
        if split == self._load_extra.Split.TEST:
            dataset = None
